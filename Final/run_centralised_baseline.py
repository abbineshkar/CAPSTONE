"""Run two centralised transformer baselines without changing E1-E20.

Baselines:
    CB-D: Centralised DistilBERT
    CB-T: Centralised TinyBERT

The runner uses the same cleaned 70/15/15 split, random seed, tokenizer
length, batch size, optimiser, learning rate, validation metric, early
stopping rule, and final IID test set as the federated experiments.

Place this file in the project root beside config.py and run:

    python run_centralised_baseline.py
    python run_centralised_baseline.py --only distilbert
    python run_centralised_baseline.py --only tinybert
    python run_centralised_baseline.py --skip_existing
"""
